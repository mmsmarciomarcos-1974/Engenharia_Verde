...
# Função para gerar Fibonacci iterativo (mais eficiente que recursão simples)
def fibonacci(n):
    a, b = 0, 1
    for _ in range(n):
        a, b = b, a + b
...
for rodada in range(1, 301):
    ...
    soma = fibonacci(35000)  # valor ajustável para intensidade

    if rodada % 50 == 0:
        hora_exec = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"\n--- Rodada {rodada} concluída --- Último Fibonacci={soma}")
